...
| **Q** | Convention enforcement | Made `digismith:brainstorming`/`digismith:writing-plans` honor DigiSmith's unified docs convention (correct folder, HTML format) instead of falling back to their own defaults, via a standalone `digismith:enforcer` skill that wrapped both from the outside. **Retired 2026-09-04** once map item W's vendoring mechanism was proven live — Enforcer's logic was absorbed directly into the vendored primitives themselves (see **W.5**) rather than kept as an external wrapper. Letter freed for future reuse, same precedent as letter R being freed and later reused for W's own predecessor numbering. |
