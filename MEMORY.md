...
| **Q** | Convention enforcement | Makes `digismith:brainstorming`/`digismith:writing-plans` honor DigiSmith's unified docs convention (correct folder, HTML format) instead of falling back to their own defaults; also carries the amendment that any HTML doc DigiSmith writes gets published via the `Artifact` tool for readability, unless the active profile has `publish_artifact: false` |
