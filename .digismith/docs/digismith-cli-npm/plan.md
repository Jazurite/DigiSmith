...
**Architecture:** A new zero-runtime-dependency workspace package. `src/index.ts` is a thin dispatcher (group table, usage, `--version`); `src/vps/` is `scripts/vps-session/` moved verbatim via `git mv`, with only its entry file changing from a process entry point to an exported `run(argv)`. Depot's skill keeps invoking the checkout path directly (unaffected by the point below); the installed binary is for humans. **Amended mid-plan:** the original "no build — Node strips `.ts` types natively" premise does not survive a real install — Node refuses to type-strip any `.ts` file under a `node_modules` directory (`ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING`, no flag lifts it, confirmed live and reproduced independently), and every package manager installs a package under `node_modules` by definition. The package now builds with `tsc` to `dist/`, and the bin points at compiled JS; the checkout-path source layout, dispatcher logic, and tests are unaffected — see the design's Overview amendment.

**Tech Stack:** TypeScript on Node ≥24, compiled with `tsc` (already a root devDependency) to plain JS for publishing; native execution (no flag) still used for the checkout path and for tests. pnpm workspace, Vitest, `node:` builtins only. No new dependencies.

## Global Constraints

- Node `>=24` — `engines.node` in the new package; no runtime version check is added.
- Zero runtime dependencies in `packages/cli/package.json`. No new devDependency either — `typescript` is already a root devDependency (`^5.7.0`) and pnpm's workspace script execution already puts the root `node_modules/.bin` on `PATH`, the same way this package's own `"test": "vitest run"` resolves a bare `vitest` with no local devDependency.
- `packages/cli/` gets its own `tsconfig.build.json` (emit-enabled, `outDir: "dist"`, `rewriteRelativeImportExtensions: true`) purely for producing the publishable JS. The root `tsconfig.json` (`noEmit: true`) still covers type-checking; its `include` gains `packages/cli/src/**/*.ts`, unchanged from the original plan.
- `dist/` is generated by `pnpm build` (aliased as `prepack`, so `pnpm pack`/`pnpm publish` always rebuild first) and is gitignored — never committed, never hand-edited.
- Package name `@digismith/cli`, version `0.1.0`, bins `digismith` and `dg` both → `dist/index.js`, `files: ["dist"]`, `publishConfig.access: "public"` — exactly as the design's (amended) `package.json`.
...
| Path | Responsibility |
|---|---|
| `packages/cli/package.json` | Package identity, bins, published files, build/prepack scripts. Create. |
| `packages/cli/tsconfig.build.json` | Emit-enabled compile config: `src/` → `dist/`. Create. |
...
| `.gitignore` | Ignore packed tarballs (`*.tgz`) and the build output (`dist/`). Modify. |
...
### Task 2: The `@digismith/cli` package — manifest, build config, dispatcher, README, and packaging verification
...
- Modify: `.gitignore` (append `*.tgz` and `dist/`)
...
- [ ] **Step 1b: Write the build config**
...
`rewriteRelativeImportExtensions` (TypeScript 5.7+, already the pinned root version) rewrites the source's `"./vps/index.ts"`-style relative imports to `"./vps/index.js"` in the emitted output, so nothing in `src/` needs to change to be buildable. This file has no test of its own — it's exercised end-to-end by Step 8's `pnpm build` and Step 9's checkout-path run against the *source* (unaffected by this file), plus Step 10 onward running against the *compiled* output.
...
- [ ] **Step 7: Ignore packed tarballs and build output, register the workspace package**
...
- [ ] **Step 9b: Build and verify the compiled output runs directly**

```bash
cd packages/cli
pnpm build
ls dist dist/vps
node dist/index.ts vps status 2>&1 | head -1; true
node dist/index.js vps status; echo "exit=$?"
node dist/index.js --version
cd ../..
```

Expected: `dist/index.js` and `dist/vps/*.js` exist (no `.ts` in `dist/`); the deliberate `dist/index.ts` probe fails with `ENOENT` (compiled output is `.js`, this line just documents that nothing named `.ts` should be runnable there — if it unexpectedly succeeds, stop and report); `node dist/index.js vps status` prints the same no-config line as Step 9 and exits 1; `node dist/index.js --version` prints `0.1.0`. This is the direct regression check for the `ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING` failure the design amendment records — running compiled JS, not a `.ts` file, must not hit it even once this file later sits under `node_modules` (Step 11).
...
`pnpm pack`'s own `prepack` step re-runs `pnpm build` first — expected output includes that build running, then the pack. Expected file list, exactly:

```
package/README.md
package/package.json
package/dist/index.js
package/dist/vps/checks.js
package/dist/vps/config.js
package/dist/vps/connect.js
package/dist/vps/index.js
package/dist/vps/run-command.js
package/dist/vps/status.js
```

No `.ts` file, no `.d.ts` file (declarations aren't requested — nothing imports this package as a library), no `*.test.js`, anywhere. If any of those appear, or a `src/` path appears instead of `dist/`, the `files`/`exclude` configuration is not being honoured — stop and report rather than adjusting the pattern by guesswork.
...
Expected: `names free`; install succeeds; `dg --version` prints `0.1.0`; both `vps status` invocations print the same no-config line as Step 9 with exit 1. This is the real end-to-end proof the design amendment exists for: the installed binaries run `dist/index.js` — a path that genuinely is a descendant of `node_modules` once installed — and must **not** raise `ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING` (they can't, since nothing under `node_modules` at this point is a `.ts` file; only `dist/*.js` is reachable there). Two distinct anomalies to watch for instead: (a) either binary prints **nothing** and exits 0 — the realpath guard in Step 4 (Task 2's dispatcher) is not matching; report it with the output of `where dg` and `readlink -f "$(where dg | head -1)"`. (b) `ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING` reappears — the build/`files`/`bin` wiring is somehow still resolving to a `.ts` path instead of `dist/*.js`; report the exact resolved path from the stack trace rather than patching blindly. Either anomaly is a stop-and-report, not a guess.
...
Expected: `names free again`; the only changes are the files this task creates/modifies (no stray `.tgz`, and no `packages/cli/dist/` — it's gitignored, so a built-but-untracked `dist/` here is correct, not a leftover to clean up).

- [ ] **Step 13: Commit**

```bash
git add packages/cli/package.json packages/cli/tsconfig.build.json packages/cli/README.md packages/cli/src/index.ts packages/cli/src/index.test.ts .gitignore pnpm-lock.yaml
...
1. Map table, V row — replace the substring
   ...
   ```
   not yet live-verified against the real box, 2026-09-11 <span class="status done">Done</span> · <strong>V.4</strong> the CLI published to npm as <code>@digismith/cli</code> (<code>digismith</code>/<code>dg</code>; <code>vps</code> is the first command group; <code>tsc</code>-built to <code>dist/</code> after a live install-time restriction ruled out shipping raw <code>.ts</code>), 2026-09-11 <span class="status done">Done</span></td>
   ```
...
3. Timeline — insert this event immediately before the closing `  </div>\n</section>\n\n<footer>` of the timeline section (i.e. after the last existing `<div class="event">…</div>`):
   ...
   ```html
       <div class="event">
         ...
         <code>dg</code>. Designed with no build step at all — Node 24 strips <code>.ts</code>
         types natively, verified against the checkout — but that premise didn't survive a real
         install: Node unconditionally refuses to type-strip any <code>.ts</code> file under a
         <code>node_modules</code> directory (<code>ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING</code>,
         confirmed live and reproduced independently with a two-line test file), and every package
         manager installs a package there. Reversed mid-implementation to a <code>tsc</code> build
         to <code>dist/</code> — no new dependency, <code>typescript</code> was already a root
         devDependency — with the bin pointing at compiled JS; the checkout path, source layout,
         and dispatcher logic never changed. Separately, pnpm's global installs go through symlinks
         and Node realpaths <code>import.meta.url</code>, so the entry guard compares real paths or
         the installed binary silently does nothing. Own semver from <code>0.1.0</code>; publishing
         stays manual and Jack's (<code>npm login</code>,
