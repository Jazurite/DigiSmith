...
- [`jira-client` CLI has no attachment-upload subcommand (only download) (I.6)](jira-client-attachment-upload-subcommand.md) — classic REST attachment upload is the working fallback for screenshots; the Windows CLI bug that blocked this is now fixed (I.5)
- [Real inline image embedding in Jira comments needs the Media Platform upload flow (I.6)](jira-media-platform-inline-images.md) — two dead ends already ruled out (classic-attachment-ID ADF node, MCP-connector markdown image); real Media API upload-context flow not yet scoped
...
- [`report-implementation`/`writing-plans` always target `report.html`/`plan.md`, colliding when a feature slug gets reused](report-implementation-filename-collision.md) — found live shipping the Screenshots/Videos section addendum (Q.1); the same feature slug's original 5-task build already occupied both filenames
