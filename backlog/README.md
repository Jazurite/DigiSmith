...
- [Post-finish hooks assume the Option-1 merge flow — direct-to-main push leaves them stranded](post-finish-hooks-direct-push-gap.md) — `ORIG_HEAD` stale/misleading when hooks are invoked by hand outside a real merge; distinct from `no-push-after-local-merge`
- [`bump-plugin-version.ts` always bumps minor, never patch](version-bump-always-minor.md) — no fix/feat distinction at all; caught live when 3 fix commits got bumped as minor, corrected by hand; TDD implementation started then deliberately deferred here
