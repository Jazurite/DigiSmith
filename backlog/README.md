...
- [`jira-client` CLI silently no-ops every subcommand on Windows](jira-client-cli-windows-entrypoint-bug.md) — `import.meta.url` entry-point guard never matches Windows paths; confirmed `check-credentials` falsely reports "ok" with blank creds
- [`jira-client` CLI has no attachment-upload subcommand (only download)](jira-client-attachment-upload-subcommand.md) — classic REST attachment upload is the working fallback for screenshots; blocked by the Windows CLI bug above
- [Real inline image embedding in Jira comments needs the Media Platform upload flow](jira-media-platform-inline-images.md) — two dead ends already ruled out (classic-attachment-ID ADF node, MCP-connector markdown image); real Media API upload-context flow not yet scoped
